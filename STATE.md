...
## 現在フェーズ
feature/product-imagesブランチ（feature/ui-polishから分岐）で商品画像アップロード機能が完了。管理者用商品CRUD画面(/admin/products/new, /admin/products/:id/edit)に画像アップロード欄を追加し、Supabase Storageへの実アップロード・公開URL取得・商品一覧/詳細ページでの画像表示までブラウザで動作確認済み。mainへは未マージ（マージ待ち）。

※feature/ui-polish・feature/stripe-webhookブランチも別途mainへマージ待ちの状態。3ブランチのマージ順序・コンフリクト解消は次のステップで対応予定

## 未確定
- 3つのfeatureブランチ（stripe-webhook・ui-polish・product-images）のmainへのマージ順序・統合
